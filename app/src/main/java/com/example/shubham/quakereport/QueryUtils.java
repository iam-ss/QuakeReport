...
public final class QueryUtils {
    ...
    public static ArrayList<EarthQuake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray levelOneJSONArray = root.getJSONArray("features");
            for(int i=0;i<levelOneJSONArray.length();i++){
                JSONObject featureObject = (JSONObject) levelOneJSONArray.get(i);
                JSONObject requiredFeatures = featureObject.getJSONObject("properties");

                String place = requiredFeatures.getString("place");
                Float mag = new Float(requiredFeatures.getDouble("mag"));
                Date date = new Date(requiredFeatures.getLong("time"));
                ...
                Log.i("QueryUtils.java",earthquake.toString());
